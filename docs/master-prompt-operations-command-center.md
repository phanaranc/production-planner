...
| นน.คงเหลือ (ตัน) | remaining_weight_ton | ว่างเปล่าในข้อมูลตัวอย่างที่พบ — **สูตรคำนวณแทนยืนยันแล้ว** (ดูหมายเหตุด้านล่าง) |
...
- ชีต "ข้อมูลรับเข้า" (อีกไฟล์ย่อย) มีรูปแบบหัวตารางเดียวกันทุกคอลัมน์ แต่แผนกบำบัดพบ
  ...
  แผนกที่ต้องรองรับในระบบจริง (ไม่ใช่แค่ TF/SP/AR/FC/SRF ของ Planner) **master list
  ยืนยันครบแล้วโดยผู้ใช้ตรง (2026-09-13)**: `SP`, `AR`, `FC`, `TF`, `046`, `RF201`,
  `RF202`, `RF206` — ไม่มีรหัสอื่นเพิ่มเติม (ปิด checklist ข้อ 2) ยังไม่ทราบความหมายของ
  `046`/`RF2xx` ในเชิงธุรกิจ (เช่น คืออะไรต่างจาก SP/AR/FC/TF) แต่ไม่ใช่ประเด็นบล็อกการ
  hardcode enum อีกต่อไปเพราะลิสต์ค่าครบแล้ว
- `นน.คงเหลือ (ตัน)` ว่างในทุกแถวตัวอย่างที่ยืนยันได้ — **สูตรคำนวณแทนยืนยันแล้วโดยผู้ใช้ตรง
  (2026-09-13)**: `remaining_weight_ton = นน.เข้า - นน.บำบัด` เฉพาะกรณีที่ทั้งสองค่ามีจริง
  เท่านั้น ถ้าค่าใดค่าหนึ่งขาดหาย **ยังคงห้ามคำนวณแทน** ให้ทำเครื่องหมาย data-incomplete
  ตามเดิม (ปิด checklist ข้อ 3)
...
2. **เสร็จแล้ว** — โครงสร้างคอลัมน์และ master list ค่า treatment_department/
   treatment_code ยืนยันครบแล้วโดยผู้ใช้ตรง (2026-09-13): `SP`, `AR`, `FC`, `TF`,
   `046`, `RF201`, `RF202`, `RF206` — ไม่มีรหัสอื่นเพิ่มเติม (§3.1) ความหมายเชิงธุรกิจของ
   `046`/`RF2xx` ยังไม่ทราบ แต่ไม่บล็อกการเขียน enum อีกต่อไป
3. **เสร็จแล้ว** — ผู้ใช้ยืนยันตรง (2026-09-13) ว่า `remaining_weight_ton = นน.เข้า -
   นน.บำบัด` เฉพาะเมื่อทั้งสองค่ามีจริง ถ้าค่าใดขาดหายยังคงห้ามคำนวณแทน (§3.1, §5)
