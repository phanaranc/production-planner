...
**`แผนงานส่งกากรายวัน.xlsx` — ยังไม่ได้เปิดไฟล์นี้** (คนละไฟล์กับด้านบน) ห้ามระบุโครงสร้าง
คอลัมน์ของไฟล์นี้เป็นข้อเท็จจริงในเอกสารจนกว่าจะเปิดอ่านจริง

ก่อนเขียน import mapping จริงสำหรับ `Plan delivery Haz 2026.xlsx`:

1. ยืนยันความหมายของจำนวน `l` ในแถว `แผน` กับผู้ใช้ (สมมุติฐาน: 1 ตัวอักษร = 1 เที่ยวรถที่
   วางแผน — ต้องถามยืนยันก่อนเขียนโค้ดแปลงเป็นตัวเลข)
2. ยืนยันว่าค่า SRF จะปรากฏในไฟล์เดือนอื่นหรือไม่ หรือสาย SRF ไม่ใช้ระบบแผนนี้เลย
3. ออกแบบ import mapping ใหม่ที่แทนที่ตรรกะเดิมใน `src/lib/csv-import/export-plan-mapping.ts`
   ทั้งหมด (ตรรกะเดิม "l = flag เดียว" ใช้ไม่ได้กับไฟล์จริง) — งานนี้กระทบ core ของฟีเจอร์แผน
   ส่งออกที่มีอยู่แล้ว **ต้องขอความเห็นชอบแนวทางจากผู้ใช้ก่อนเริ่มเขียนโค้ดจริง**
4. เปิดไฟล์ `แผนงานส่งกากรายวัน.xlsx` จริงก่อนอ้างอิงโครงสร้างของมันในที่ใดๆ
...
1. `Plan delivery Haz 2026.xlsx` เปิดและยืนยันโครงสร้างแล้ว (2026-09-13, §3.3) — **แต่ยังต้อง
   ยืนยันความหมายจำนวน `l` ในแถวแผน และแนวทาง import ก่อนเขียนโค้ดจริง (ผลกระทบสูงต่อ
   ฟีเจอร์แผนส่งออกที่มีอยู่แล้ว)**. `แผนงานส่งกากรายวัน.xlsx` **ยังไม่ได้เปิด** (§3.3)
